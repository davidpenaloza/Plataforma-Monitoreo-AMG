...
- **Fuente NotPII_Interpolated:** valida las últimas 5 señales del job `mlp-prd-caj-pisystem-job01` en una ventana de 30 minutos. Alerta si las 5 últimas señales son fallas.
...
- Funciones dominio desacopladas: `fn_uat_mlp_svfn_dom_fuente_notpii_interpolated_status(startTime, endTime)`, `fn_uat_mlp_svfn_dom_tags_status(startTime, endTime)`, `fn_uat_mlp_svfn_dom_transformacion_status(startTime, endTime)`, `fn_uat_mlp_svfn_dom_prediccion_status(startTime, endTime)` y `fn_uat_mlp_svfn_dom_resumen_status(startTime, endTime)`.
...
```kusto
fn_uat_mlp_svfn_dom_resumen_status(ago(1h), now())
| project NotPII_Interpolated, Tags, Transformacion, Prediccion, EstadoGlobal, color
```
